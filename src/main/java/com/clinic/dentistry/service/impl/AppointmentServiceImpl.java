        ...
        try {
            mailService.sendNotification(
                    "Уважаемый(ая), " + appointment.getClient().getFullName() + ",\n" +
                            "Спасибо за выбор клиники Santé et Dents для вашего стоматологического приема. Мы рады видеть Вас в нашем учреждении."
                            + "\nВаша запись подтверждена со следующими деталями:\nДата и время приема: " + appointment.getDate().format(formatter) +
                            "\nВрач: " + appointment.getDoctor().getFullName() + " по должности " + appointment.getDoctor().getJobTitle() +
                            "\n\nПросим Вас прийти за 10 минут до указанного времени и иметь при себе все необходимые документы и страховую карту.\n" +
                            "Спасибо за Ваше доверие. Мы стремимся обеспечить Вам качественное обслуживание." +
                            "\n\nС уважением,\n" +
                            "Команда Santé et Dents",
                    appointment.getClient().getEmail(),
                    "Запись на прием в клинику Santé et Dents"
            );
            ...
            if (isCanCancel(user, appointment)) {
                ...
                try {
                    mailService.sendNotification(
                            "Уважаемый(ая), " + appointment.getClient().getFullName() + ",\n" +
                                    "Спасибо за выбор клиники Santé et Dents для вашего стоматологического приема. Мы рады видеть Вас в нашем учреждении."
                                    + "\nВаша запись успешно отменена со следующими деталями:\nДата и время приема: " + appointment.getDate().format(formatter) +
                                    "\nВрач: " + appointment.getDoctor().getFullName() + " по должности " + appointment.getDoctor().getJobTitle() +
                                    "\n\nС уважением,\n" +
                                    "Команда Santé et Dents",
                            appointment.getClient().getEmail(),
                            "Отмена записи на прием в клинику Santé et Dents"
                    );
                } catch (MailException ignored) {
                }
