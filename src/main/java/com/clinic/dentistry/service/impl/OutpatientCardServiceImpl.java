        ...
        if (form.getEmail() != null && !form.getEmail().equals(card.getEmail())) {
                try {
                    mailService.sendNotification(
                            "Уважаемый(ая), " + form.getFullName() + "," +
                                    "Ваш E-mail успешно изменен на сервисе Santé et Dents.\n" +
                                    "\nВаш логин: " + form.getUsername()  +
                                    "\nВаш пароль: " + form.getPassword() +
                                    "\n\nС уважением, \n" +
                                    "Команда Santé et Dents",
                            form.getEmail(),
                            "Успешная смена e-mail на сайте Santé et Dents"
                    );
                    mailService.sendNotification(
                            "Уважаемый(ая), " + form.getFullName() + "," +
            ...
            if (form.getUsername() != null && form.getPassword() != null) {
                try {
                    mailService.sendNotification(
                            "Уважаемый(ая), " + form.getFullName() + "," +
                                    "\n\nВаши логин и пароль успешно исменены на сервисе Santé et Dents.\n" +
                                    "\nВаш логин: " + form.getUsername() +
                                    "\nВаш пароль: " + form.getPassword() +
                                    "\n\nС уважением, \n" +
                                    "Команда Santé et Dents",
                            form.getEmail(),
                            "Успешная смена логина и пароля на сайте Santé et Dents"
                    );
                } catch (MailException ignored) {
                }
            }
